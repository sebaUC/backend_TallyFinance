...
export class BotV3Service {
  ...
  constructor(
    ...
    try {
      ...
      this.promptTemplate =
        'Eres Gus, asistente financiero de TallyFinance. Tono: {tone}. Mood: {mood}.';
  ...
  private checkQuickResponse(text: string, tone: string): string | null {
    if (BotV3Service.DASHBOARD_PATTERN.test(text)) {
      const template =
        BotV3Service.DASHBOARD_MSGS[tone] ||
        BotV3Service.DASHBOARD_MSGS.neutral;
  ...
  private sanitizeEditChanges(
    ...
    return Object.keys(updated)
      ...
      .map((k) => {
        ...
        if (typeof prev === 'string' && BotV3Service.UUID_RE.test(prev))
          prev = undefined;
        if (typeof next === 'string' && BotV3Service.UUID_RE.test(next))
          next = undefined;
