...
const ANT_CATEGORIES =
  /café|coffee|snack|golosin|dulce|chicle|bebida|jugo|agua|galleta|completo|sopaipilla|mote|empanada|helad|vending/i;

function isAntExpense(
  amount: number,
  category: string,
  name?: string,
): boolean {
...
async function computePostExpenseContext(
  ...
  const catMonthTotal = (catMonthExpenses || []).reduce(
    (s: number, t: any) => s + Number(t.amount),
    0,
  );
...
function findCategory(
  ...
  for (const cat of categories) {
    const catNorm = stripAccents(cat.name.toLowerCase());
    const syns = SYNONYMS[catNorm];
  ...
  for (const cat of categories) {
    ...
    for (const [canonical, syns] of Object.entries(SYNONYMS)) {
      if (catNorm === canonical && syns.some((s) => isSimilar(lower, s, 2))) {
        return { type: 'synonym', category: cat };
      }
    }
  }
