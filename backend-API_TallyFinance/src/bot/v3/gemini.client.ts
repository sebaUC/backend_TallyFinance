...
export class GeminiClient {
  ...
  async chat(
    ...
    let loops = 0;
    while (response.response.functionCalls()?.length && loops < MAX_FUNCTION_LOOPS) {
      loops++;
      ...
      for (const fc of fnCalls) {
        ...
        functionsCalled.push({ name: fc.name, args: fc.args as Record<string, any>, result: fnResult });
        fnResponses.push({
