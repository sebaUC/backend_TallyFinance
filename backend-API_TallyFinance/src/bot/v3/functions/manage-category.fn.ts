...
export async function manageCategory(
  ...
  switch (operation) {
    ...
    case 'create': {
      ...
      if (existing?.length) {
        return { ok: true, data: { already_existed: true, category: existing[0] } };
      }
      ...
      if ((count ?? 0) >= 50) {
        return { ok: false, error: 'MAX_CATEGORIES', message: 'Máximo 50 categorías' };
      }
    ...
    case 'rename': {
      if (!name || !new_name) return { ok: false, error: 'NAME_AND_NEW_NAME_REQUIRED' };
      ...
      if (!cat) return { ok: false, error: 'NOT_FOUND', message: `Categoría "${name}" no existe` };
      ...
      if (error) return { ok: false, error: 'DB_ERROR', message: error.message };
      return { ok: true, data: { operation: 'rename', old_name: name, new_name: new_name.trim() } };
    }
    ...
    case 'delete': {
      ...
      if (error) return { ok: false, error: 'DB_ERROR', message: error.message };
      return { ok: true, data: { operation: 'delete', name, transactionsUnlinked: count ?? 0 } };
    }
    ...
    case 'update_budget': {
      if (!name || budget === undefined) return { ok: false, error: 'NAME_AND_BUDGET_REQUIRED' };
    ...
    default:
      return { ok: false, error: 'UNKNOWN_OPERATION', message: `Operación "${operation}" no válida` };
  }
}
